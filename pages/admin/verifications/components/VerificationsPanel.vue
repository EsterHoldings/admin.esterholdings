    ...
    <section
      ...
      <article
        ...
        <div class="verification-request-card__identity">
          <div
            class="verification-request-card__avatar"
            :class="{ 'is-online': requestItem.user.is_online }">
            <img
...
<script lang="ts" setup>
  ...
  const normalizedRequestScope = computed<RequestScope>(() =>
    props.requestScope === "payout" ? "payout" : "identity"
  );
  ...
  const handleMarkedNotificationsByTypes = (payload?: { types?: string[]; verificationScope?: RequestScope }): void => {
...
<style lang="scss" scoped>
  ...
  .verification-request-card__avatar {
    ...
    background: linear-gradient(
      135deg,
      var(--ui-primary-main),
      color-mix(in srgb, var(--ui-primary-main) 60%, #000000)
    );
  }
  ...
  .verification-request-card__avatar::after {
    content: "";
    position: absolute;
    right: -1px;
    bottom: 2px;
  ...
  .verification-request-card__avatar.is-online::after {
    opacity: 1;
    transform: scale(1);
  }
