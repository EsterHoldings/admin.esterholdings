    ...
    <PrimeCard
      ...
      <template #content>
        <div class="client-accounts-empty__body">
          ...
          <span>{{
            text("admin.clients.accounts.emptySubtitle", "Client MT accounts will appear here after creation.")
          }}</span>
        </div>
      </template>
    </PrimeCard>
          ...
          <div class="client-accounts-row">
            ...
            <div class="client-accounts-row__grid">
              <div class="client-data-item">
                <span class="client-data-item__label">{{
                  text("admin.clients.accounts.columns.balance", "Balance")
                }}</span>
              ...
              <div class="client-data-item">
                <span class="client-data-item__label">{{
                  text("admin.clients.accounts.columns.leverage", "Leverage")
                }}</span>
                <strong class="client-data-item__value">{{ account.leverage_display || "-" }}</strong>
              </div>
              <div class="client-data-item">
                <span class="client-data-item__label">{{
                  text("admin.clients.accounts.columns.paymentType", "Payment type")
                }}</span>
                <strong class="client-data-item__value">{{ account.payment_type || "-" }}</strong>
              </div>
              <div class="client-data-item">
                <span class="client-data-item__label">{{
                  text("admin.clients.accounts.columns.createdAt", "Created at")
                }}</span>
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          text("admin.accounts.messages.refreshError", "Failed to refresh account balance.")
      );
    } finally {
