<template>
  <div class="client-verification">
    ...
    <div
      class="client-verification__anchors"
      aria-hidden="true">
    ...
    <PrimeCard
      ...
      <template #content>
        <div class="verification-empty-card__body">
          ...
          <span>{{
            text(
              "admin.verifications.clientTimeline.emptySubtitle",
              "Client changes and admin actions will appear here."
            )
          }}</span>
        </div>
      </template>
        ...
        <template #content>
          <div class="verification-timeline-card__inner">
            ...
            <div
              ...
              <span
                class="inline-flex items-center gap-1.5 rounded-full bg-[color:color-mix(in_srgb,var(--ui-primary-main)_10%,transparent)] px-2 py-1 font-semibold text-[color:var(--ui-text-main)]">
                ...
                <button
                  ...
                  @click="
                    copyVerificationValue(
                      item.actorEmail,
                      text('admin.verifications.clientTimeline.messages.emailCopied', 'Email copied')
                    )
                  ">
              ...
              <button
                ...
                @click="
                  copyVerificationValue(
                    item.actorId,
                    text('admin.verifications.clientTimeline.messages.actorIdCopied', 'Actor ID copied')
                  )
                ">
            ...
            <div
              v-if="item.comment"
              class="verification-timeline-card__comment">
              {{ item.comment }}
            </div>
      ...
      <template #footer>
        <div class="verification-confirm-dialog__footer">
          ...
          <PrimeButton
            ...
            :label="
              timelineDecisionDialog.status === 'approved'
...
<script lang="ts" setup>
  ...
  const hasUnreadVerificationSignal = (userId: string, section?: VerificationSectionTarget): boolean =>
    unreadVerificationNotifications.value.some(
      item => item.userId === userId && (section === undefined || item.section === section)
    );
  ...
  const loadVerificationData = async () => {
    ...
    try {
      ...
      verificationHistoryVisibleCount.value = Math.min(
        Math.max(verificationHistoryVisibleCount.value, 10),
        Math.max(verificationHistoryRows.value.length, 10)
      );
      ...
      const paymentRows = Array.isArray(paymentsResponse?.data?.data?.data) ? paymentsResponse.data.data.data : [];
  ...
  const requestHasProfileToReview = (request: ClientVerificationRequestRow): boolean => request.profile_review_required;
  ...
  const hasPendingProfile = computed(() =>
    Boolean(activePendingRequest.value && requestHasProfileToReview(activePendingRequest.value))
  );
  const hasPendingDocuments = computed(() =>
    Boolean(activePendingRequest.value && requestHasDocumentsToReview(activePendingRequest.value))
  );
  const hasPendingPayout = computed(() =>
    Boolean(activePendingRequest.value && requestHasPayoutToReview(activePendingRequest.value))
  );
  const hasPendingRequests = computed(() =>
    clientRequestRows.value.some(request => request.request_state === "pending")
  );
  ...
  const pendingDocuments = computed(() =>
    documentsListRequestData.value.filter(document => document.state === "pending")
  );
  ...
  const isInitialTimelineLoading = computed(
    () => isAnyLoading.value && verificationHistoryRows.value.length === 0 && clientRequestRows.value.length === 0
  );

  const timelineStatusFilterOptions = computed<
    Array<{
  ...
  const requestFocusItems = (
    request: ClientVerificationRequestRow
  ): Array<{
  ...
  const currentRequestDocumentPreviews = computed(() =>
    documentsListRequestData.value.map(documentPreviewMeta).slice(0, 2)
  );
  ...
  const historyComment = (row: VerificationHistoryRow): string => {
    const raw = row.data?.comment;

    return typeof raw === "string" ? raw.trim() : "";
  };

  const historyDocumentPreviews = (
  ...
  const visibleHistoryActorTypes = new Set(["admin", "client", "system"]);
  ...
  const historyStepLabel = (row: VerificationHistoryRow): string => {
    const step = String(row.data?.step ?? "")
      .trim()
      .toLowerCase();
  ...
  const historyDescription = (row: VerificationHistoryRow): string => {
    ...
    switch (rowKey) {
      case "profile_submitted":
        return text(
          "admin.verifications.clientTimeline.historyDescriptions.profileSubmitted",
          "Client submitted profile data."
        );
      case "documents_uploaded":
        return text(
          "admin.verifications.clientTimeline.historyDescriptions.documentsSubmitted",
          "Client uploaded documents."
        );
      case "document_deleted":
        return text(
          "admin.verifications.clientTimeline.historyDescriptions.documentDeleted",
          "Client removed a document."
        );
      case "payout_detail_submitted":
      case "payout_detail_updated":
        return text(
          "admin.verifications.clientTimeline.historyDescriptions.payoutSubmitted",
          "Client submitted payment details."
        );
  ...
  const documentTimelineDocuments = (documents: VerificationDocumentItem[]): VerificationTimelineDocument[] =>
    documents.map(document => ({
      id: document.id,
      label:
        document.name || document.document_data.number || text("admin.verifications.documents.defaultName", "Document"),
      url: document.document_data.full_url,
      preview: documentPreviewMeta(document),
    }));
  ...
  const pendingTimelineItems = computed<VerificationTimelineItem[]>(() => {
    ...
    if (request && requestHasProfileToReview(request)) {
      ...
      items.push({
        ...
        actor: latestProfileHistory
          ? historyActorText(latestProfileHistory)
          : text("admin.verifications.history.actorClient", "Client"),
        actorId: "",
        actorType: "client",
    ...
    pendingDocuments.value.forEach(document => {
      items.push({
        ...
        description: text(
          "admin.verifications.clientTimeline.items.documentDescription",
          "Client uploaded a document for identity verification."
        ),
    ...
    pendingPayoutDetails.value.forEach(paymentDetail => {
      items.push({
        ...
        title:
          paymentDetail.name ||
          text("admin.verifications.clientTimeline.items.payoutTitle", "Payment detail requires review"),
        description:
          paymentDetail.paymentSystemName ||
          text(
            "admin.verifications.clientTimeline.items.payoutDescription",
            "Client submitted or updated payment details."
          ),
        actor: text("admin.verifications.history.actorClient", "Client"),
        actorId: "",
        actorType: "client",
        ...
        comment: paymentDetail.adminComment.trim(),
  ...
  const historyTimelineItems = computed<VerificationTimelineItem[]>(() =>
    verificationHistoryRows.value.filter(shouldShowVerificationHistoryRow).map(row => {
      ...
      return {
        ...
        comment: historyComment(row),
        unread: false,
      };
    })
  );

  const timelineSectionMatches = (
    item: VerificationTimelineItem,
    filter: VerificationTimelineSectionFilter
  ): boolean => {
    if (filter === "all") {
      return true;
    }
  ...
  const rawVerificationTimelineItems = computed(() =>
    [...pendingTimelineItems.value, ...historyTimelineItems.value.slice(0, verificationHistoryVisibleCount.value)].sort(
  ...
  const verificationSummaryCards = computed(() => {
    ...
    return [
      ...
      },
      {
        ...
        hint: text(
          "admin.verifications.clientTimeline.summary.pendingPaymentDetails",
          "Payment details waiting for moderation"
        ),
      },
      {
        id: "history",
  ...
  const visibleVerificationTimelineItems = computed(() =>
    rawVerificationTimelineItems.value.filter(
      item =>
  ...
  const verificationTabs = computed(() => [
    {
      ...
      needsAttention:
        ...
        hasUnreadProfileSignals.value ||
        hasUnreadDocumentsSignals.value,
    },
    {
      id: "payout" as const,
  ...
  const getTimelineUpdateKey = (item: VerificationTimelineItem): string =>
    `${item.actionType || "history"}:${item.actionId}`;
  ...
  const timelineDecisionDialogMessage = computed(() => {
    ...
    if (item.actionType === "document") {
      return status === "approved"
        ? text("admin.verifications.confirm.documentApprove", "Approve this document?")
        : text("admin.verifications.confirm.documentReject", "Reject this document?");
    }
  ...
  const paymentDetailPrimaryEntries = (
    paymentDetail: AdminPaymentDetailItem
  ): Array<{ key: string; label: string; value: string }> => {
    const fieldsPayload = paymentDetail.data?.fields;
    const source =
      fieldsPayload && typeof fieldsPayload === "object" && !Array.isArray(fieldsPayload)
  ...
  const paymentDetailLegacyEntries = (
    paymentDetail: AdminPaymentDetailItem
  ): Array<{ key: string; label: string; value: string }> => {
    ...
    const paysystem =
      legacyPayload.paysystem && typeof legacyPayload.paysystem === "object" && !Array.isArray(legacyPayload.paysystem)
    ...
    if (systemName) {
      rows.push({
        key: "paysystem",
        label: text("admin.verifications.payout.legacyFields.system", "System"),
        value: systemName,
      });
    }
    ...
    if (legacyType) {
      rows.push({
        key: "type",
        label: text("admin.verifications.payout.legacyFields.type", "Type"),
        value: legacyType,
      });
    }
    ...
    if (legacyStatus) {
      rows.push({
        key: "status",
        label: text("admin.verifications.payout.legacyFields.status", "Legacy status"),
        value: legacyStatus,
      });
    }
    ...
    if (legacyId) {
      rows.push({
        key: "old_requisite_id",
        label: text("admin.verifications.payout.legacyFields.id", "Legacy ID"),
        value: legacyId,
      });
    }
    ...
    if (legacyComment) {
      rows.push({
        key: "comment",
        label: text("admin.verifications.payout.legacyFields.comment", "Legacy comment"),
        value: legacyComment,
      });
    }
  ...
  watch(() => [hasPendingProfile.value, hasPendingDocuments.value, hasPendingPayout.value], clearResolvedHighlight);
...
<style lang="scss" scoped>
  ...
  .verification-card--highlighted {
    ...
    box-shadow:
      0 0 0 1px rgba(73, 108, 222, 0.16),
      0 18px 44px rgba(6, 18, 63, 0.18);
  }
  ...
  .verification-timeline-card__comment {
    padding: 12px 14px;
    border-radius: 14px;
    border: 1px solid rgba(255, 255, 255, 0.06);
    background: rgba(255, 255, 255, 0.04);
  ...
  .verification-client-request-card.is-unread-notification {
    ...
    box-shadow:
      0 0 0 1px rgba(87, 132, 255, 0.16),
      0 10px 28px rgba(16, 38, 120, 0.12);
  }
  ...
  .verification-empty-card {
    ...
    background:
      radial-gradient(
        circle at 16% 0%,
        color-mix(in srgb, var(--verification-accent) 10%, transparent),
        transparent 38%
      ),
  ...
  .verification-filter-strip {
    ...
    border: 1px solid var(--verification-glass-border);
    border-radius: 16px;
    background: linear-gradient(145deg, var(--verification-glass-bg), var(--verification-glass-bg-strong));
    box-shadow: var(--verification-glass-shadow);
