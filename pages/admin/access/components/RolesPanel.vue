<template>
  <div class="access-entity-panel">
    ...
    <div
      v-if="isLoading && rolesData.length === 0"
      class="access-entity-panel__loading">
      <UiIconSpinnerDefault />
    </div>

    <div
      v-else-if="rolesData.length === 0"
        ...
        <div class="access-entity-card__top">
          <div>
            ...
            <div class="access-entity-card__subtitle">
              ...
              {{ resolveText("admin.access.components.roles-panel.permissionsCount", "permissions") }}
            </div>
          </div>
        ...
        <div class="access-entity-card__grid">
          <div class="md:col-span-2">
            <div class="access-entity-card__label">
              {{ t("admin.access.components.roles-panel.columns.permissions") }}
            </div>
            <div
              v-if="role.permissions.length"
              class="access-entity-card__chips">
              <span
...
<script lang="ts" setup>
  ...
  const canCreateRoles = computed(
    () => adminAuthStore.hasRole("super-admin") || adminAuthStore.hasPermission("create-roles")
  );
  const canUpdateRoles = computed(
    () => adminAuthStore.hasRole("super-admin") || adminAuthStore.hasPermission("update-roles")
  );
  const canDeleteRoles = computed(
    () => adminAuthStore.hasRole("super-admin") || adminAuthStore.hasPermission("delete-roles")
  );
